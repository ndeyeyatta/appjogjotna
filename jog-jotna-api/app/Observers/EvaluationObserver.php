...
class EvaluationObserver {
    public function created(Evaluation $eval): void {
        ...
        if (Alerte::where('evaluation_id', $eval->id)->exists()) return;
        $alerte = Alerte::create([
            'enfant_id' => $eval->enfant_id,
            'evaluation_id' => $eval->id,
            'type_alerte' => $eval->dimension,
            'niveau' => $eval->urgent ? 'urgent' : 'modere',
            'message' => "Retard chez {$eval->enfant->nom_complet}. {$eval->dimension_libelle}. Score:{$eval->score_global}%.",
            'statut' => 'active',
        ]);
